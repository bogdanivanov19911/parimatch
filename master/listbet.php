<?php
	...
	date_default_timezone_set("Europe/Moscow");
				...
				if(!empty($_POST["reject"])) {
					$query97 = $db->query("SELECT `id`,`price`,`user_id` FROM `placed_bet` WHERE `id` = '?i' AND `bet_status` = '0' LIMIT 1", $_POST["reject"]);
					...
					$balance_reject = $row714["balance"] + $row97["price"];
					
					$db->query("UPDATE `placed_bet` SET `bet_status` = '4' WHERE `id` = '?i'", $row97["id"]);
					$db->query("UPDATE `users` SET `balance` = '?i' WHERE `id` = '?i'", $balance_reject,$row714["id"]);
				}
				...
				if($_GET["status"] == 1) {
					...
					foreach($row11 as $key11 => $value11) {
						...
						if($value11["bet_status"] == 1) {
							...
							$status = '<span style="color: #ffec6b;">В игре</span><br/>
			...
			$tpl = new template('master/template/listbet.tpl');
