...
$result = file_get_contents("http://185.43.223.70/engine/cron/api/data/result.json");
        ...
        foreach ($value2["evts"] as $key3 => $value3) {
            ...
            if ($db->getAffectedRows() == 1) {
                ...
                if (!empty($value3["sc_ev"]) OR !empty($value3["sc_ext_ev"])) {
                    $db->query("UPDATE `events` SET `result` = true, `score` = '?s', `score_all` = '?s', `curr_time` = '?s' WHERE `parse_id` = '?i' LIMIT 1", $value3["sc_ev"], $value3["sc_ext_ev"], $curr_time->format('Y-m-d H:i:s'), $value3["id_ev"]);
                }
            }
        }
    }
