<?php
	...
	$tournamentId = $_POST["idleague"];
	
	$date_events = date("Y-m-d H:i:s", strtotime("+3 hour"));
	...
	$query = $db->query('SELECT *,DATE_FORMAT(time_start + INTERVAL "?s" HOUR ,"%d/%m|%Y|%H:%i|%Y-%m-%d %H:%i:%s") as `date_start` FROM `events` WHERE `result` IS NULL AND `is_live` = 0 AND `time_start` >= "'.$date_events.'" AND `tournament_id` = "?i" ORDER BY `time_start`',$_COOKIE['hour'],$tournamentId);
		...
		while ($rows = $query->fetch_assoc()) {
			...
			$tournament = "";	
				...
				while($row2 = $query2->fetch_assoc()) {
					$tourn = new template($_SERVER["DOCUMENT_ROOT"].'/template/line-tournament-prematch.tpl');
					$events = "";
						...
						foreach($events_array[$row2["id"]] as $row3) {
							...
							$event->set('{tournament_name}', str_replace($rows["name"].". ","",$row2["name"]));
					...
					$tourn->set('{date_start}', $date_start[0]);
					$tourn->set('{game_name}',$rows["name"]);
					$tourn->set('{idG}', $rows["id"]);
